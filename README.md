...
#### socket
IP层就是把数据分组从一台计算机搬到另一台，搬运服务一点都不可靠，丢包、重复、失序的情况时有发生，而TCP层可以通过失败重传来保证可靠性，但是建立TCP连接非常复杂，需要三次握手，滑动窗口、累积确认、分组缓存、流量控制等，最后四次分手，这些都属于操作系统内核的部分，应用程序没必要重复开发，socket是对TCP/IP协议的抽象层，每个连接就是两个端点，每个端点就是一个socket，即客户端的IP和port，服务器端的IP和port
